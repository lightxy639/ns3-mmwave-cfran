...
class MmWaveBearerStatsCalculator : public LteStatsCalculator
{
  public:
    ...
    /**
     * Gets the time of last data reset, available when m_externalReset is true
     * @param imsi IMSI of the UE
     * @param lcid LCID
     * @return the time of last data reset, available when m_externalReset is true
     */
    Time GetLastImsiLcidResetTime(uint64_t imsi, uint8_t lcid);
  ...
  private:
    ...
    /**
     * Reset time of each (imsi, lcid), available when m_externalReschedule is true
     */
    std::map<ImsiLcidPair_t, Time> m_resetTime;
