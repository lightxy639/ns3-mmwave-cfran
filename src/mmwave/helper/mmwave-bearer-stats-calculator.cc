    ...
    static TypeId tid =
        TypeId("ns3::MmWaveBearerStatsCalculator")
            ...
            .AddAttribute("AggregatedStats",
                          ...
                          MakeBooleanChecker())
            .AddAttribute("ExternalReschedule",
                          "Allow other classes to reset recorded results.",
                          BooleanValue(false),
                          MakeBooleanAccessor(&MmWaveBearerStatsCalculator::m_externalReschedule),
                          MakeBooleanChecker());
    return tid;
}
...
MmWaveBearerStatsCalculator::ResetResultsForImsiLcid(uint64_t imsi, uint16_t lcid)
{
    ...
    m_resetTime[ImsiLcidPair_t(imsi, lcid)] = Simulator::Now();
}
...
Time
MmWaveBearerStatsCalculator::GetLastImsiLcidResetTime(uint64_t imsi, uint8_t lcid)
{
    NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
    ImsiLcidPair_t p(imsi, lcid);

    auto it = m_resetTime.find(p);
    if (it == m_resetTime.end())
    {
        return Seconds(0.);
    }
    else
