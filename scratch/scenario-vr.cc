...
main(int argc, char* argv[])
{
    ...
    edgeContainer.Create(nEdgeNodes);
    centralContainer.Create(nCentralNodes);
    remoteHostContainer.Add(edgeContainer);
    remoteHostContainer.Add(centralContainer);
    ...
    double x, y;
    double nConstellation = nGnbNodes;
    ...
    for (int8_t i = 0; i < nConstellation; ++i)
    {
        ...
        else
        {
            ...
            gnbPositionAlloc->Add(Vector(centerPosition.x + x, centerPosition.y + y, bsHeight));
        }
        ...
        for (uint32_t j = 0; j < ues; j++)
        {
            ...
            uePositionAlloc->Add(Vector(utX, utY, utHeight));
        }
    }
    ...
    // Ptr<ListPositionAllocator> enbPositionAlloc = CreateObject<ListPositionAllocator>();
    // // enbPositionAlloc->Add (Vector ((double)mmWaveDist/2 + streetWidth, mmw1Dist +
    // // 2*streetWidth, mmWaveZ));
    ...
    // -5, 1.6));
    // ueNodes.Get(0)->GetObject<ConstantVelocityMobilityModel>()->SetVelocity(Vector(0, 0,
    // 0)); ueNodes.Get(1)->GetObject<MobilityModel>()->SetPosition(
    //     Vector(60, 70, 1.6));
    // ueNodes.Get(1)->GetObject<ConstantVelocityMobilityModel>()->SetVelocity(Vector(0, 0,
    // 0)); Install mmWave, lte, mc Devices to the nodes
    ...
    // Simulator::Schedule(Seconds(transientDuration + 2), &CfApplication::MigrateUeService,
    // cfApp, 1, 3); Simulator::Schedule(Seconds(1.5),
